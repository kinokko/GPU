...
void HistogramGPU(int* d_hist_out, unsigned char* d_img_in, int img_size, int nbr_bin) {
	...
	//Count the color
	HistogramGPUAction << <numSMs * 32, threadsPerBlock, threadsPerBlock*sizeof(unsigned char) >> >(d_hist_out, d_img_in, img_size);
}
...
__global__ void HistogramGPUAction(int * histOut, unsigned char * imgIn, int imgSize) {
	...
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < imgSize; i += blockDim.x * gridDim.x) {
		...
		pixels[threadIdx.x] = imgIn[i];
		//atomicAdd(&histOut[pixels[threadIdx.x]], 1);
		atomicAdd(&loc_hist[pixels[threadIdx.x]], 1);
	}
	__syncthreads();
	for (int i = threadIdx.x; i < 256; i += blockDim.x) {
		atomicAdd(&histOut[i], loc_hist[i]);
	}
	__syncthreads();
}
...
void HistogramEqualizationGPU(unsigned char * img_out, int * d_lut_in, unsigned char * d_img_in, int img_size){
	...
	HistogramEqualizationGPUAction << <64, 1024, 1024*sizeof(unsigned char) >> > (d_img_out, d_lut_in, d_img_in, img_size);
...
__device__ unsigned char  GetFinalPixel(unsigned char pixel, int* lut) {
	if (lut[pixel] > 255){
		...
		return (unsigned char)lut[pixel];
	}
}
